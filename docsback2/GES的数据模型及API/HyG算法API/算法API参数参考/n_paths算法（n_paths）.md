...
> 章节路径：持久化版 / HyG算法API / 算法API参数参考 / n_paths算法（n_paths）
> 来源：https://support.huaweicloud.com/intl/zh-cn/api-ges/ges_03_0526.html

## n_paths算法（n_paths）
#### 功能介绍
根据输入参数，执行n_paths算法。
n_paths算法用于寻找图中两节点之间在层关系内的n条路径。
#### URI
```
...
n |  否 |  Integer |  路径个数，取值范围[1,100] ，默认值为10。
k |  否 |  Integer |  层数，取值范围[1,10]，默认值为5。
#### 响应参数
参数 |  类型 |  说明
---|---|---
...
paths |  List |  source节点和target节点之间的路径，格式： [[path1],[path2], …] 其中，路径（path）的格式可参考：[最短路径（Shortest Path）](https://support.huaweicloud.com/intl/zh-cn/api-ges/ges_03_0447.html)。
