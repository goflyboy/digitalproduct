...
> 章节路径：持久化版 / HyG算法API / 算法API参数参考 / louvain算法（louvain）
> 来源：https://support.huaweicloud.com/intl/zh-cn/api-ges/ges_03_0507.html
...
weight |  否 |  String |  边上权重，取值为空或字符串， 当图中的边没有配置该属性时，算法会报错。
  * 空：边上的权重、距离默认为“1"。
  * 字符串：对应的边上的属性将作为权重。
