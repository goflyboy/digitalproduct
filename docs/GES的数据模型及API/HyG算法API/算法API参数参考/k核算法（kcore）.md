...
> 章节路径：持久化版 / HyG算法API / 算法API参数参考 / k核算法（kcore）
> 来源：https://support.huaweicloud.com/intl/zh-cn/api-ges/ges_03_0452.html
...
| 参数 | 是否必选 | 类型 | 说明 |
---|---|---|---
k |  是 |  Integer |  核数，算法会返回核数大于等于k的节点。取值范围大于等于0。
#### 响应参数
参数 |  类型 |  说明
---|---|---
...
coreness |  List |  各节点的coreness值（coreness>=k），格式：[{vertexId:corenessValue},...]。其中：
  * vertexId：string类型。
  * corenessValue：int类型。
