...
│ ProductModel ──constrainedBy──> ConstraintRule                        │
│ ProductInstance(S1110/S22) ──realizes/cuts──> ProductModel/Part       │
└─────────────────────────────┬────────────────────────────────────────┘
                              │ configure
                              ▼
┌──────────────────────────────────────────────────────────────────────┐
...
| OntoLinkInst | 两个对象实例之间的关系 | link_type_id, source_inst_id, target_inst_id, sequence, effective_from/to | 两端类型符合 OntoLinkType |
...
| PartType | 具体 Part 的通用业务语义 | code, name, part_kind, identifier_policy | instanceOf -> PartClassType |
| ConstraintRuleType | 兼容、依赖、选择、数量、计算和优选规则类型 | rule_kind, scope, cardinality, severity, expression_schema | constrains -> 产品、分类或规格 |

PartClass 是分类和候选集边界，Part 是可选的具体项。用面向对象语言类比时，Part 更准确地说是 PartClass 的实例，而不是每个物料都通过程序类继承 PartClass；否则新增物料会变成类结构变更。
...
| PartOfferingType | Part 在可售产品中的可用性和裁剪结果 | enabled, default_selected, min/max_qty, fixed, sales_status | offers -> PartType |
| SpecValueType | 将规格定义赋为具体值 | value, unit, source, override_reason | valueOf -> SpecDefinition |
| OfferingRuleType | 仅对某可售产品生效的增量规则 | rule_kind, expression, priority, effective_period | overrides/augments -> ConstraintRuleType |

ProductInstance 不宜使用程序类继承 ProductClass。它应通过 realizes 关系绑定一个已发布的 ProductModelVersion，并保存裁剪及覆盖差异。这样才能定位继承基线、回放历史配置，避免源模型发布后悄然改变在售产品。

### 1.5 L2：业务模型实例

#### 1.5.1 产品模型

ProductModel 是某产品族或产品平台的可复用骨架。例如“X86 双路机架服务器平台”。

| 实体 | 定义 | 关键字段 | 关系 |
| --- | --- | --- | --- |
| ProductModel | 产品平台或产品族模型 | id, code, name, product_class_type_id, owner, status | 1:N -> ProductModelVersion |
| ProductModelVersion | 不可变的已发布模型基线 | id, model_id, version, status, effective_from/to | 1:N -> PartClass、ConstraintRule |
| PartClass | 模型中的部件分类或子模块 | id, code, name, parent_id, selection_policy, min/max_qty, multi_instance | 1:N -> Part、SpecDefinition；可递归 |
| Part | 分类中的具体候选部件或物料 | id, code, name, part_class_id, price_ref, max_qty, status | 1:N -> SpecValue |
| SpecDefinition | 产品或部件规格定义 | code, data_type, unit, value_domain, required, scope | 1:N -> SpecOption |
| SpecValue | Part 的具体规格值 | part_id, spec_definition_id, value, unit | N:1 -> Part |
| InputParameter | 用户需求、计算值或汇总值 | code, type, assign_type, min/max, aggregate_type | 挂载产品或 PartClass |
| ConstraintRule | 硬约束、计算和优选逻辑 | code, rule_kind, scope, expression, priority, severity | 引用分类、Part、Spec 或 Parameter |

#### 1.5.2 产品实例

ProductInstance 是已经确定销售边界的可售产品。对标电商模型时，它更接近可配置 SPU，而不是最终库存 SKU。

| 实体 | 定义 | 关键字段 |
| --- | --- | --- |
| ProductInstance | 可售产品，如 S1110/S22 | id, code, name, model_version_id, market, status, effective_from/to |
| PartOffering | 对源 Part 的包含、排除、默认和数量裁剪 | product_instance_id, part_id, enabled, default_selected, fixed, min/max_qty |
| SpecOverride | 产品实例层属性覆盖 | target_type/id, spec_id, value, reason |
| RuleOverride | 禁用、替换或追加实例规则 | source_rule_id, action, replacement_rule_id, reason |
| ChannelOffering | 渠道和区域可售性 | channel, region, currency, status, effective_period |

裁剪应保存差异而不是复制完整 ProductModel。有效模型由“已发布模型基线 + PartOffering + SpecOverride + RuleOverride”合并得到。
...
ProductModel 1 ── N ProductModelVersion
ProductModelVersion 1 ── N PartClass
PartClass 1 ── N PartClass (recursive composition)
PartClass 1 ── N Part
PartClass 1 ── N SpecDefinition
Part 1 ── N SpecValue
ProductModelVersion 1 ── N ConstraintRule

ProductInstance N ── 1 ProductModelVersion
ProductInstance 1 ── N PartOffering N ── 1 Part
ProductInstance 1 ── N SpecOverride / RuleOverride
...
| LT001 | ProductModel | containsPartClass | PartClass | 1:N |
| LT002 | PartClass | containsPart | Part | 1:N |
| LT003 | ProductInstance | realizes | ProductModelVersion | N:1 |
| LT004 | ProductInstance | offers | Part | N:M |
...
1. 用 L0 创建 ProductClassType、PartClassType、PartType 和 ProductInstanceType。
2. 明确组合、实例化、引用和裁剪关系。
...
1. 选择已发布 ProductModelVersion 作为基线。
2. 创建 S1110/S22，填写型号、市场、渠道和有效期。
3. 通过 PartOffering 包含或排除 Part，设置默认、固定和数量边界。
4. 仅通过 SpecOverride/RuleOverride 保存必要差异。
...
ProductModelVersion  ──compile──> Module
...
| --- | --- | --- |
| ProductModelVersion | Module | Module 含版本、PartCategory、Part、Para 和 Rule，是执行投影 |
...
| ProductModelVersion | DRAFT -> VALIDATED -> PUBLISHED -> RETIRED；PUBLISHED 不可变 |
...
| 产品数据架构师 | 维护 ProductModel、PartClass、Spec 和 Rule |
...
2. 以服务器 CPU + drive 为首个纵向切片，跑通 ProductModel、S1110 裁剪、配置求解和 BOM 输出。
3. 建立 ProductModelSnapshot 到 cruleengine.Module 的单向编译器，并保存源 ID 与执行 code 的映射。
